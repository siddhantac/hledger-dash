...
def test_included_file_change_invalidates_cache(tmp_path):
    # The real journal is an umbrella that `include`s per-year files, and
    # `git pull` only rewrites the files that actually changed — so the
    # umbrella's own mtime stays frozen at clone time while the data under it
    # moves every sync. Keying the cache on HLEDGER_FILE's mtime alone served
    # permanently stale charts in that setup; the key must track includes too.
    main = tmp_path / "main.journal"
    included = tmp_path / "2024.journal"
    included.write_text("2024-01-01 * Test\n    expenses:food  10.00 USD\n    assets:checking\n")
    main.write_text("include 2024.journal\n")
    main_mtime_at_clone = os.path.getmtime(main)

    old_file = os.environ["HLEDGER_FILE"]
    os.environ["HLEDGER_FILE"] = str(main)
    _clear_caches()
    try:
        assert _count_subprocess_calls(lambda: hl.get_expense_total("2024-01", "2024-01")) == 1
        assert _count_subprocess_calls(lambda: hl.get_expense_total("2024-01", "2024-01")) == 0

        # Append to the *included* file only, exactly as a sync would.
        time.sleep(0.01)
        included.write_text(
            included.read_text()
            + "\n2024-01-02 * More\n    expenses:food  5.00 USD\n    assets:checking\n"
        )
        assert os.path.getmtime(main) == main_mtime_at_clone, "umbrella must stay untouched"

        assert _count_subprocess_calls(lambda: hl.get_expense_total("2024-01", "2024-01")) == 1
        assert hl.get_expense_total("2024-01", "2024-01") == 15.0
    finally:
        os.environ["HLEDGER_FILE"] = old_file
        _clear_caches()
